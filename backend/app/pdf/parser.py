...
_VISION_SYSTEM = """You are the Design Office plan reader. Extract the floor plan geometry
from the attached image at 2576 px. Return a strict JSON payload matching the
schema at the end. Numbers must be expressed in the same units you see on the
plan; if a scale bar is visible, report it so the caller can rescale. If you
...
{
  "scale_label": "...",            // raw text of any scale annotation found (e.g. "1:200")
  "envelope_points_px": [[x, y], ...],  // outer polygon in image pixel space
  "columns_px": [{"cx": x, "cy": y, "r": radius_px}, ...],
  "cores": [
    {"kind": "elevator|wc|shaft|mep|stair",
     "points_px": [[x,y], ...]}
  ],
  "windows_px": [{"x1": ..., "y1": ..., "x2": ..., "y2": ..., "facade": "north|south|east|west|unknown"}],
  "stairs_px": [{"points_px": [[x,y], ...]}],
  "text_labels": ["...", "..."],
  "uncertainties": ["...", "..."]
}
...
- Only include a column if you are at least 80% confident.
- Classify facades by position against the envelope bounding box (top=north, bottom=south).
- Return valid JSON.
"""


def call_vision_hd(png_bytes: bytes, client: ClaudeClient | None = None, tag: str = "pdf.vision") -> dict:
...
def fuse(
    ...
    # Windows come from either Vision HD, or from detected line runs along the
    # envelope's north/south edges in the PyMuPDF-only path.
    windows_out: list[Window] = []
    if vision and image_size:
        for w in vision.get("windows_px", []):
        ...
        for line in vectors["lines"]:
            x1_pt, y1_pt_ = line["x1"], line["y1"]
            x2_pt, y2_pt_ = line["x2"], line["y2"]
            a_mm = to_plan_mm(x1_pt, y1_pt_)
            b_mm = to_plan_mm(x2_pt, y2_pt_)
            dx = abs(a_mm[0] - b_mm[0])
            dy = abs(a_mm[1] - b_mm[1])
            length = math.hypot(dx, dy)
            # Short horizontal segments on the top or bottom edge are windows.
