...
| Path | Size | Role |
|------|------|------|
| `opus-4-7/README.md` | 2.0 kB | Handoff doc for the coding agent. Tells us to read chats first, then `Design Office.html` top-to-bottom, then follow imports. States the prototype is HTML/CSS/JS (not production) — we recreate in whatever target technology fits. Explicitly says **don't** open the prototype in a browser for screenshots ; read sources directly. |
