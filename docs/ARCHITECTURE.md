...
│           /testfit  → Three-variant Test Fit                      │
│           /justify  → Sourced argumentaire + A4 PDF               │
│           /export   → A1 DXF with Design Office layers            │
│                                                                   │
│  Components : PlanSvg (envelope + columns + cores + variant zones)│
│  Local state : Test Fit result persisted to localStorage so       │
│                Justify + Export pick it up across page changes    │
└───────────────────────┬───────────────────────────────────────────┘
                        │ HTTP (JSON)
                        ▼
┌─ BACKEND ─────────────────────────────────────────────────────────┐
...
│  │   run_with_consolidator pattern reused by all 3 levels         │
│  │                                                                │
│  ├ app/surfaces/                                                  │
│  │   brief.py       (Level-1 : 3 sub-agents + consolidator)       │
│  │   testfit.py     (Level-2 : 3 variants + reviewers + replay)   │
│  │   justify.py     (Level-3 : 4 researchers + consolidator + PDF)│
...
│  │                                                                │
│  └ app/data/                                                      │
│      resources/   10 MCP Markdown resources (2 700 lines sourced) │
...
## 2. Three levels of managed-agent orchestration
...
call. Each level has a distinct intent :

### Level 1 — Programme synthesis (Surface 1 Brief)
...
### Level 2 — Variant generation (Surface 2 Test Fit)
...
- All three variants produced 112 – 130 postes
...
### Level 3 — Research & Cite (Surface 3 Justify)
...
`backend/app/data/resources/` holds 10 Markdown files, 2 700 lines total.
Every number cited traces back to a URL or carries `[À VÉRIFIER]`.

| Resource | Lines | Headline content |
|----------|-------|------------------|
| `office-programming.md` | 137 | NIA / FTE ratios, programme split, meeting mix, Leesman anchor |
| `acoustic-standards.md` | 480 | NF S 31-080, NF S 31-199, ISO 3382-3, Hongisto, masking playbook |
| `pmr-requirements.md` | 120 | Arrêté 20 avril 2017, widths, adapted WC, turning circles |
| `erp-safety.md` | 145 | ERP type W, category thresholds, exits, alarm systems |
| `ergonomic-workstation.md` | 164 | NF EN 527-1, EN 12464-1, R. 4223, ventilation |
| `neuroarchitecture.md` | 517 | Browning 14, Kellert 24, Nieuwenhuis, Ulrich, Kaplan, Taylor, Heerwagen |
| `flex-ratios.md` | 355 | Leesman 2019-2024, Gensler 2020-2024, sector medians |
| `furniture-brands.md` | 213 | 41 SKUs across 18 typologies, manufacturer URLs |
| `collaboration-spaces.md` | 151 | Phone booth → town hall sizings and adjacency rules |
| `biophilic-office.md` | 430 | Density tiers, species library by light, maintenance |
